...
const MODULES_SORT_MAP = { name: 'name', code: 'code' };
...
async function getAll(req, res) {
  try {
    ...
    const orderBy = resolveSort(req.query, MODULES_SORT_MAP, 'id DESC');

    if (!pagination) {
      const result = await req.db.query(`SELECT * FROM onec_modules ORDER BY ${orderBy}`);
      return res.json({ data: result.rows });
    }

    const [rows, count] = await Promise.all([
      req.db.query(`SELECT * FROM onec_modules ORDER BY ${orderBy} LIMIT $1 OFFSET $2`, [pagination.limit, pagination.offset]),
      req.db.query('SELECT COUNT(*)::int AS total FROM onec_modules')
    ]);
