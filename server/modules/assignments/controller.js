...
const BASE_SELECT = `
  SELECT a.id, a.title, a.description, a.module_id, a.due_date::text AS due_date,
         ...
         a.target_type, a.instructions, a.publish_marks, a.created_at, a.created_by,
...
async function listAssignments(req, res) {
  try {
    ...
    const groupBy = 'GROUP BY a.id, m.name, u.username, inst.first_name, inst.last_name';
...
async function getAssignment(req, res) {
  try {
    const { id } = req.params;
    const result = await req.db.query(
      `${BASE_SELECT} WHERE a.id = $1 GROUP BY a.id, m.name, u.username, inst.first_name, inst.last_name`,
      [id]
    );
...
async function createAssignment(req, res) {
  try {
    ...
    const {
      ...
      target_type, cohort_ids, student_user_ids, status,
    } = parsed.data;
    ...
    const result = await req.db.query(
      `INSERT INTO onec_assignments
         (title, description, module_id, due_date, eval_type, max_score, passing_marks,
          pass_grade, instructions, target_type, status, created_by)
      ...
      [title, description ?? null, module_id, due_date, eval_type,
       ...
       instructions ?? null, target_type, status, req.user.userId]
    );
  ...
  try {
    ...
    await req.db.query(
      `UPDATE onec_assignments SET
         ...
         target_type=$10, status=$11
       WHERE id=$12`,
      [title, description ?? null, module_id, due_date, eval_type,
       eval_type === 'marks' ? (max_score ?? 100) : null,
       eval_type === 'marks' ? (passing_marks ?? null) : null,
       eval_type === 'grades' ? (pass_grade ?? null) : null,
       instructions ?? null, target_type, status, id]
    );
  ...
  try {
    ...
    const result = await req.db.query(
      ...
      [`${s.title} (Copy)`, s.description, s.module_id, s.due_date, s.eval_type,
       s.max_score, s.passing_marks, s.pass_grade, s.instructions, s.target_type, req.user.userId]
    );
