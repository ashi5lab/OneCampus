...
async function getAll(req, res) {
  try {
    ...
    const orderBy = resolveSort(req.query, INSTRUCTORS_SORT_MAP, 'i.id DESC');

    if (!pagination) {
      const result = await req.db.query(`SELECT i.*, u.username, u.profile_picture_url ${baseQuery} ORDER BY ${orderBy}`, params);
    ...
    const [rows, count] = await Promise.all([
      req.db.query(
        `SELECT i.*, u.username, u.profile_picture_url ${baseQuery} ORDER BY ${orderBy} LIMIT $${pageParams.length - 1} OFFSET $${pageParams.length}`,
        pageParams
      ),
...
async function create(req, res) {
  try {
    ...
    const { username: providedUsername, email, staff_id, first_name, last_name, phone, meta, module_ids, cohort_ids } = parsed.data;
    ...
    try {
      ...
      for (const cohort_id of cohort_ids) {
        await req.db.query(
          'INSERT INTO onec_instructor_cohorts (instructor_id, cohort_id) VALUES ($1, $2) ON CONFLICT DO NOTHING',
          [instructor.id, cohort_id]
        );
      }
