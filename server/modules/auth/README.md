...
**Session lifetime policy** (client-enforced, see `client/src/contexts/AuthContext.jsx`): an installed PWA (Android/desktop "Add to Home Screen", or iOS Safari standalone — detected via `client/src/lib/pwa.js`'s `isStandalone()`) stays signed in indefinitely, exactly per the server-side rules above (rotation keeps sliding the refresh token's expiry forward on every use; only real 400-day inactivity, an admin-forced logout, or a password change ends it — see `revokeAllUserTokens` in `server/lib/refreshTokens.js`). Any session that is *not* an installed PWA — a regular browser tab, mobile Chrome with the site merely open — additionally auto-logs-out after 20 minutes with no mouse/keyboard/touch/scroll activity, a client-side idle timer that calls the same `/auth/logout` as a manual logout (so the refresh token is actually revoked, not just forgotten client-side). Mount-time session restore (trading the refresh cookie for a fresh access token on page load) also retries through transient network failures with connectivity-aware backoff (~14s worst case) rather than treating them as "no session" — a real 401 (genuinely invalid/expired/revoked token) still fails immediately with no retries.
