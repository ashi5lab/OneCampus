...
Last updated: end of session that produced PR #3 (`feature/purple-theme`, not yet merged).
...
- Open: PR #3 (`feature/purple-theme`) — merge this before starting new work, or rebase onto it.
...
Two dev tenants already exist in that database, provisioned via `server/scripts/provisionTenant.js`:
- `dev.onecampus.local` (school) — has a seeded `test_admin` / `password123` user, one test learner, one test instructor, one evaluation.
...
| 7 | Permissions system (replace inline role checks) | ❌ **Not started.** No `permissions` table, no `can`/`cannot`/`hasPermission` helpers. Right now the only access control is "does a valid JWT exist for this tenant" — there is no role-based restriction anywhere (an `instructor`-role user can currently hit every admin CRUD endpoint a `admin`-role user can). This is the biggest functional gap and should probably be next. |
...
- **No user-creation UI anywhere.** Creating a learner or instructor requires an existing `onec_users.id`, and there's no form to create that user — only `server/scripts/seedTestUser.js` (CLI-only) does it. This blocks real usage of the Learners/Instructors "Add" forms for anyone without direct DB/script access. Likely the single highest-leverage next task.
...
1. **Merge PR #3**, or rebase new work on top of it.
2. **User-creation UI** (backend: needs a new endpoint, e.g. `POST /api/v1/users` or fold into instructor/learner creation as a nested step; frontend: a real signup form instead of requiring a pre-existing `onec_users.id`). Unblocks actually using the app.
3. **Phase 7 — Permissions system.** Add a `permissions` table (role → permission mapping, tenant-overridable per spec §7), `can`/`cannot`/`hasPermission` helpers, and wire them into every existing controller (currently zero role-based restriction beyond "valid JWT for this tenant" exists — any authenticated user can do anything). This is the biggest correctness gap in the current build.
4. **Attendance-marking and score-entry UI** — backend already supports both; this is pure frontend work following the existing feature patterns.
5. **Security baseline items from §5b** — at minimum rate-limiting on `/auth/login` and an audit log table are cheap, high-value, and explicitly required by spec §11.
6. **Get the open decision in §6 resolved** before touching the Purple-portal screens/modules.
7. Phase 8 (kindergarten activity log) and Phase 9 (certificates) — spec-scoped, not started, lower urgency than 3–5 above.
8. Basic test coverage — pick a framework (nothing installed yet) and at minimum cover the tenant-isolation and cross-tenant-JWT-rejection behavior in §4, since that's the thing most likely to silently regress.
