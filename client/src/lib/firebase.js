...
// Two-tone "ding" for incoming foreground notifications — generated via
// Web Audio instead of shipping an audio asset (no file to host/cache-bust,
// works offline, trivially small). Browsers require a prior user gesture
// before any audio can play; by the time a push notification arrives the
// user has already interacted with the page (clicked into the app), so
// AudioContext is unlocked in practice. Fails silently if not (e.g. a
// notification arriving on a completely untouched fresh tab).
export function playNotificationChime() {
  try {
    ...
    fcmWarn('playNotificationChime() failed (likely no user gesture yet)', error);
  }
}
