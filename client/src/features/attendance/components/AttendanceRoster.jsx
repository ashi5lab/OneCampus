...
export function AttendanceRoster({ lockedCohortId, embedded = false }) {
  ...
  return (
    <div className="bg-bg min-h-screen pb-32 font-body relative">
      ...
      <div className="mt-4 pb-36">
        ...
        <div className="sticky top-0 z-40 bg-bg pt-4 pb-3 px-4 space-y-3 border-b border-border-subtle shadow-[0_4px_10px_-4px_rgba(0,0,0,0.05)] -mx-4 sm:mx-0 sm:px-0 sm:border-0 sm:shadow-none sm:pb-0">
            ...
            {searchOpen && (
              <div className="absolute top-full left-0 right-0 bg-surface rounded-2xl shadow-xl border border-border-subtle mt-1 z-50 overflow-hidden">
                <div className="flex items-center gap-2 px-4 py-3 border-b border-border-subtle">
                  ...
                  />
                  {searchQuery && (
                    <button onClick={() => setSearchQuery('')} className="text-ink-300 hover:text-ink-700 text-lg leading-none">✕</button>
                  )}
                </div>
                  ...
                  {filteredRoster.length === 0 ? (
                    ...
                    filteredRoster.slice(0, 15).map((l) => (
                      <button
                        ...
                        className="w-full flex items-center gap-3 px-4 py-2.5 hover:bg-accent-light transition border-b border-border-subtle last:border-0 text-left"
                      >
                        <Avatar name={`${l.first_name} ${l.last_name}`} src={l.photo_url} size={36} />
                        <div className="min-w-0">
                          <div className="text-[14px] font-semibold text-ink-900 truncate">{l.first_name} {l.last_name}</div>
                          <div className="text-[12px] text-ink-500">{l.registry_no || 'N/A'}</div>
                        </div>
                      </button>
                    ))
                  )}
          ...
          {/* Table Header */}
          <div className="bg-accent-light rounded-xl px-4 py-2.5 flex items-center text-[12px] font-semibold text-ink-500">
        ...
        {/* Student Rows */}
        {loadingRoster ? (
          <div className="text-center py-10 text-ink-300 text-[14px]">Loading students...</div>
        ) : paginatedRoster.length === 0 ? (
          <div className="text-center py-10 text-ink-300 text-[14px]">No students found.</div>
        ) : (
          <div className="bg-surface md:rounded-2xl md:shadow-sm border-0 md:border border-border-subtle divide-y divide-surface-muted -mx-4 md:mx-0">
            {paginatedRoster.map((learner, idx) => {
              ...
              return (
                <div key={learner.id} className="px-4 py-3 flex items-center gap-3">
                  <div className="text-[13px] text-ink-300 w-8 text-center flex-shrink-0">{globalIdx}</div>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-[12px] text-ink-500">{learner.registry_no || 'N/A'}</div>
                  </div>
                  ...
                  {canMark ? (
                    <div className="relative flex-shrink-0">
                      <button
                        ...
                        className="flex items-center gap-1.5 border border-border rounded-xl px-3 py-1.5 bg-surface min-w-[105px] justify-between"
                      >
                      ...
                      {openStatusId === learner.id && (
                        <div
                          onMouseDown={(e) => e.stopPropagation()}
                          className="absolute right-0 top-full mt-1 bg-surface rounded-xl shadow-xl border border-border-subtle z-50 overflow-hidden min-w-[120px]"
                        >
                          {STATUS_OPTIONS.map((opt) => (
                            <button
                              ...
                              className={`w-full flex items-center gap-2 px-4 py-2.5 text-[13px] hover:bg-surface-muted transition
                                ${currentStatus === opt.value ? 'font-bold bg-surface-muted' : ''}`}
                            >
        ...
        {/* Pagination */}
        {totalStudents > 0 && (
          <div className="bg-surface rounded-2xl px-4 py-3 shadow-sm border border-border-subtle">
            <div className="flex items-center justify-between flex-wrap gap-3">
              <div className="flex items-center gap-2 text-[12px] text-ink-500">
                ...
                <select
                  ...
                  className="border border-border rounded-lg px-2 py-1 text-[12px] text-ink-700 focus:outline-none"
                >
              ...
              <div className="flex items-center gap-1.5">
                ...
                <button
                  ...
                  disabled={page === 1}
                  className="w-7 h-7 rounded-lg border border-border flex items-center justify-center disabled:opacity-40 hover:bg-surface-muted transition"
                >
                ...
                {visiblePages.map((p) => (
                  <button
                    ...
                    className={`w-7 h-7 rounded-lg text-[12px] font-bold transition
                      ${p === page ? 'bg-accent text-white' : 'border border-border text-ink-700 hover:bg-surface-muted'}`}
                  >
                    {p}
                  </button>
                ))}
        ...
        {totalStudents > pageSize && (
          <button
            ...
            className="w-full bg-surface border border-border rounded-2xl py-3.5 text-[14px] font-semibold text-accent flex items-center justify-center gap-2 shadow-sm"
          >
      ...
      {canMark && roster.length > 0 && (
        <div className="fixed bottom-24 md:bottom-8 left-0 right-0 z-50 flex justify-center px-4 pointer-events-none">
          <button
            ...
            className="w-full max-w-[300px] bg-accent text-white rounded-full py-3.5 text-[15px] font-bold shadow-xl shadow-accent/30 flex items-center justify-center gap-2 active:scale-[0.99] transition-transform disabled:opacity-70 pointer-events-auto"
          >
