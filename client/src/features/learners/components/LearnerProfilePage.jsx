    ...
    <div className="w-full overflow-hidden">
      ...
      <svg viewBox={`0 0 ${width} ${height}`} className="w-full h-auto drop-shadow-sm">
        ...
        {[0, 25, 50, 75, 100].map(val => (
          <g key={val}>
            <text x="0" y={getY(val) + 4} fontSize="10" fill="var(--ink-300)">{val}</text>
            <line x1={padding} y1={getY(val)} x2={width} y2={getY(val)} stroke="var(--border)" strokeWidth="1" strokeDasharray="4 4" />
          </g>
        ))}
        {/* Line */}
        ...
        {pointsData.map((val, i) => (
          <circle key={i} cx={getX(i)} cy={getY(val)} r="4" fill="var(--accent)" stroke="var(--surface)" strokeWidth="2" />
        ))}
  ...
  return (
    <div className="relative flex items-center justify-center" style={{ width: size, height: size }}>
      <svg width={size} height={size} className="transform -rotate-90">
        ...
        {/* Present circle */}
        <circle
          cx={center} cy={center} r={radius} fill="none" stroke="var(--success)"
            ...
            <div className="p-6 md:p-8">
                        ...
                        {sameNumber ? (
                          <div className="flex items-center gap-3 bg-white/10 px-4 py-2 rounded-full border border-white/10 shadow-sm">
                            <div className="flex items-center gap-2">
                              <a href={`tel:${primaryGuardianPhone}`} className="flex items-center justify-center w-7 h-7 bg-surface text-accent rounded-full transition-colors shadow-sm hover:bg-accent-light" title="Call">
                          ...
                          <div className="flex flex-col gap-2">
                            {primaryGuardianPhone && (
                              <div className="flex items-center gap-3 bg-white/10 px-4 py-1.5 rounded-full border border-white/10 shadow-sm">
                                <a href={`tel:${primaryGuardianPhone}`} className="flex items-center justify-center w-6 h-6 bg-surface text-accent rounded-full transition-colors shadow-sm hover:bg-accent-light" title="Call">
              ...
              <div className="mt-8 grid grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6 lg:gap-8 pt-8 border-t border-white/10 relative">
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <p className="text-[11px] text-white/80 font-medium mb-0.5">CGPA</p>
                    <p className="text-xl font-extrabold leading-none mb-1">{avgScorePct != null ? `${(avgScorePct/10).toFixed(1)}/10` : '—'}</p>
                    <p className="text-[11px] font-bold text-success">{avgScorePct >= 80 ? 'Excellent' : 'Average'}</p>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-4 relative">
                  ...
                  <div>
                    <p className="text-[11px] text-white/80 font-medium mb-0.5">Class Rank</p>
                    <p className="text-xl font-extrabold leading-none mb-1">{learner.meta?.rank || 'N/A'} <span className="text-sm font-medium text-white/70">/ {learner.meta?.total_students || '—'}</span></p>
                    <p className="text-[11px] font-bold text-white/70">Top 11%</p>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-4 relative">
                  ...
                  <div>
                    <p className="text-[11px] text-white/80 font-medium mb-0.5">Attendance</p>
                    <p className="text-xl font-extrabold leading-none mb-1">{attendanceRate != null ? `${attendanceRate}%` : '—'}</p>
                    <p className="text-[11px] font-bold text-white/70">{attendanceRate >= 90 ? 'Good' : 'Needs Impr.'}</p>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-4 relative">
                  ...
                  <div>
                    <p className="text-[11px] text-white/80 font-medium mb-0.5">Days Present</p>
                    <p className="text-xl font-extrabold leading-none mb-1">{attendanceCounts.present || 0} <span className="text-sm font-medium text-white/70">/ {totalAttendance}</span></p>
                    <p className="text-[11px] font-bold text-white/70">This Month</p>
                  </div>
                </div>
          ...
          <div className="flex flex-col xl:flex-row gap-6">
            ...
            <div className="flex-1 min-w-0 space-y-6">
                ...
                <div className="space-y-6">
                  ...
                  <div className="bg-surface rounded-2xl shadow-sm border border-border p-5">
                    <div className="flex items-center justify-between mb-4">
                      ...
                      <button onClick={() => setTab('academics')} className="text-xs font-bold text-accent flex items-center hover:underline">
                        ...
                        {subjectScores.length > 0 ? subjectScores.slice(0, 5).map((sub, i) => (
                          <div key={i} className="flex items-center justify-between py-3 px-2 -mx-2 hover:bg-surface-muted/30 transition-colors rounded-lg">
                            <div className="flex items-center gap-3 text-sm font-semibold text-ink-800">
                              <div className={`w-2 h-2 rounded-full ${i % 2 === 0 ? 'bg-success' : 'bg-accent'}`}></div>
                              {sub.name}
                            </div>
                            <span className={`text-sm font-bold ${sub.pct >= 80 ? 'text-success' : sub.pct >= 50 ? 'text-warning' : 'text-danger'}`}>
                              {sub.pct}/100
                            </span>
                          </div>
                        )) : (
                  ...
                  <div className="bg-surface rounded-2xl shadow-sm border border-border p-5">
                    <div className="flex items-center justify-between mb-4">
                      ...
                      <button onClick={() => setTab('attendance')} className="text-xs font-bold text-accent flex items-center hover:underline">
                        View all <ChevronRight className="w-3 h-3 ml-0.5" />
                      </button>
                    </div>
                    ...
                    <div className="flex flex-col sm:flex-row items-center gap-8">
                      ...
                      <div className="flex-1 divide-y divide-border/50 w-full text-sm">
                        <div className="flex items-center justify-between py-2.5">
                          <div className="flex items-center gap-2 text-ink-700 font-medium"><div className="w-2 h-2 rounded-full bg-success"></div> Present</div>
                          <span className="font-bold text-ink-900">{attendanceCounts.present || 0} Days</span>
                        </div>
                        <div className="flex items-center justify-between py-2.5">
                          <div className="flex items-center gap-2 text-ink-700 font-medium"><div className="w-2 h-2 rounded-full bg-danger"></div> Absent</div>
                          <span className="font-bold text-ink-900">{attendanceCounts.absent || 0} Days</span>
                        </div>
                        <div className="flex items-center justify-between py-2.5">
                          <div className="flex items-center gap-2 text-ink-700 font-medium"><div className="w-2 h-2 rounded-full bg-warning"></div> Late</div>
                          <span className="font-bold text-ink-900">{attendanceCounts.late || 0} Days</span>
                        </div>
                      </div>

                      <div className="bg-accent-light/50 rounded-xl p-4 border border-accent-light min-w-[140px] text-center w-full sm:w-auto">
                        <p className="text-[11px] text-accent font-bold uppercase tracking-wider mb-2">Overall Total</p>
                        <p className="text-2xl font-extrabold text-accent-dark">{totalAttendance}</p>
                        <p className="text-[11px] font-semibold text-accent mt-1">Days Logged</p>
                      </div>
                    </div>
                  </div>
                      ...
                      <div className="flex items-center gap-3">
                        {behaviorStats.count === 0 ? (
                          ...
                          <span className={`text-sm font-bold px-2 py-1 rounded-md flex items-center gap-1.5 ${
                            behaviorStats.score >= 90 ? 'bg-success-light text-success' :
                            behaviorStats.score >= 70 ? 'bg-warning-light text-warning' :
              ...
              {tab === 'more' && (
                <div className="space-y-4">
                  ...
                  {canManage && (
                    <button onClick={handleDelete} className="w-full bg-danger-light rounded-xl border border-danger/20 p-4 flex items-center justify-between hover:bg-danger/15 transition-colors text-left">
            ...
            <div className={`w-full xl:w-[320px] shrink-0 flex-col gap-5 ${tab === 'overview' ? 'flex' : 'hidden xl:flex'}`}>
              ...
              <div className="bg-surface rounded-2xl shadow-sm border border-border p-5">
                <div className="flex items-center justify-between mb-4">
                  <h3 className="text-[13px] font-bold uppercase tracking-wider text-ink-900 flex items-center gap-2">
                    <Users className="w-4 h-4 text-accent" /> Parent & Guardian
                  </h3>
                ...
                {guardians.length === 0 ? (
                  ...
                  <div className="divide-y divide-border/50">
                    ...
                    <div className="pt-3">
                      <button onClick={() => setShowGuardianLinks(true)} className="w-full text-center text-xs font-bold text-accent hover:underline">
                        View Full Details →
                      </button>
                    </div>
                  </div>
              ...
              <div className="bg-surface rounded-2xl shadow-sm border border-border p-5">
                <div className="flex items-center justify-between mb-4">
                  <h3 className="text-[13px] font-bold uppercase tracking-wider text-ink-900 flex items-center gap-2">
                    <BookOpen className="w-4 h-4 text-accent" /> Academic Details
                  </h3>
                </div>
              ...
              <div className="bg-surface rounded-2xl shadow-sm border border-border p-5">
                <h3 className="text-[13px] font-bold uppercase tracking-wider text-ink-900 flex items-center gap-2 mb-4">
                  <Grid className="w-4 h-4 text-accent" /> Quick Links
                </h3>
                <div className="grid grid-cols-2 gap-3">
                  <button onClick={() => setTab('academics')} className="bg-surface-muted hover:bg-accent-light rounded-xl p-3 flex flex-col items-center justify-center gap-2 text-center transition-colors border border-transparent hover:border-accent-light">
                    <FileText className="w-5 h-5 text-success" />
                    <span className="text-xs font-bold text-ink-700">Report Card</span>
                  </button>
                  <button className="bg-surface-muted hover:bg-accent-light rounded-xl p-3 flex flex-col items-center justify-center gap-2 text-center transition-colors border border-transparent hover:border-accent-light opacity-60 cursor-not-allowed">
