...
- **UserSearchSelect**: For any user selection input (student, teacher, guardian, staff), the `<UserSearchSelect />` autocomplete component MUST be used instead of standard HTML dropdowns. Users are fetched via `useAllUsers()` and filtered by `roles`.
